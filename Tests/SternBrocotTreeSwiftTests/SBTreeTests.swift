...
class SBTreeTests: XCTestCase {
    ...
    func testInitFromSpecifiedHeight1() throws {
        let tree = try SBTree<Rational>(height: 1)
        XCTAssertEqual(tree.nodes.map(\.description), ["1/1"])
    }

    func testInitFromSpecifiedHeight2() throws {
        let tree = try SBTree<Rational>(height: 2)
        XCTAssertEqual(tree.nodes.map(\.description), ["1/2", "1/1", "2/1"])
    }

    func testInitFromSpecifiedHeight3() throws {
        let tree = try SBTree<Rational>(height: 3)
        XCTAssertEqual(tree.nodes.map(\.description), ["1/3", "1/2", "2/3", "1/1", "3/2", "2/1", "3/1"])
    }

    func testInitFromSpecifiedHeight4() throws {
        let tree = try SBTree<Rational>(height: 4)
        XCTAssertEqual(tree.nodes.map(\.description), ["1/4" ,"1/3", "2/5", "1/2", "3/5", "2/3", "3/4", "1/1", "4/3", "3/2", "5/3", "2/1", "5/2", "3/1", "4/1"])
    }

    func testInitEnoughRoomCase() throws {

        // Around 2 ^ 3
        try XCTContext.runActivity(named: "Before Boundary Value 2 ^ 3") { (_) in
            let tree = try SBTree<Rational>(numberOfNodes: 7)
            XCTAssertEqual(tree.nodes.count, 7)
        }

        try XCTContext.runActivity(named: "Next Boundary Value 2 ^ 3") { (_) in
            let tree = try SBTree<Rational>(numberOfNodes: 8)
            XCTAssertEqual(tree.nodes.count, 15)
        }

        // Around 2 ^ 4
        try XCTContext.runActivity(named: "Before Boundary Value 2 ^ 4") { (_) in
            let tree = try SBTree<Rational>(numberOfNodes: 15)
            XCTAssertEqual(tree.nodes.count, 15)
        }

        try XCTContext.runActivity(named: "Next Boundary Value 2 ^ 4") { (_) in
            let tree = try SBTree<Rational>(numberOfNodes: 16)
            XCTAssertEqual(tree.nodes.count, 31)
        }

    }

    func testNodesInDepth1() throws {
        let nodes = try SBTree<Rational>.nodesInDepth(1)
        XCTAssertEqual(nodes.map(\.description), ["1/1"])
    }

    func testNodesInDepth2() throws {
        let nodes = try SBTree<Rational>.nodesInDepth(2)
        XCTAssertEqual(nodes.map(\.description), ["1/2", "2/1"])
    }

    func testNodesInDepth3() throws {
        let nodes = try SBTree<Rational>.nodesInDepth(3)
        XCTAssertEqual(nodes.map(\.description), ["1/3", "2/3", "3/2", "3/1"])
    }

    func testNodesInDepth4() throws {
        let nodes = try SBTree<Rational>.nodesInDepth(4)
    ...
    func testSimplicity() throws {
        for i in 1...5 {
            let nodes = try SBTree<Rational>.nodesInDepth(i)
